...
export function VocabAnswerButton({ choice, prefix, state, accentColor, onPress }: Props) {
  const { C } = useTheme();
  ...
  return (
    <Animated.View style={[styles.wrapper, { transform: [{ translateX: shakeX }, { scale }], opacity }]}>
      <TouchableOpacity
      ...
      >
        ...
        <Text style={[styles.choice, { color: textColor }]} numberOfLines={2} adjustsFontSizeToFit>
          {choice}
        </Text>
      </TouchableOpacity>
    </Animated.View>
...
const styles = StyleSheet.create({
  ...
  choice: { fontSize: 15, fontWeight: '700', flex: 1, textAlign: 'center' },
});
