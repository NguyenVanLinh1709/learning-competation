...
export default function ColorSetupScreen({ navigation }: Props) {
  const { setConfig } = useColorPerceptionStore();
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        >
          ...
          <View style={styles.section}>
            <View style={[styles.playerTag, { backgroundColor: '#7C3AED' }]}>
              <Text style={styles.playerTagText}>{t.yourName}</Text>
            </View>
            <TextInput
              style={[styles.input, { borderColor: '#7C3AED', backgroundColor: C.surface, color: C.text }]}
          ...
          >
            <LinearGradient
              colors={canStart ? ['#7C3AED', '#A855F7'] : ['#888', '#999']}
