...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'SoloResult'> };
    ...
    <LinearGradient colors={G.home} style={styles.outer}>
      <View style={[styles.scroll, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 40 }]}>
        ...
        <Animated.View style={[styles.banner, { transform: [{ scale: bannerScale }] }]}>
          ...
          <Text style={[styles.completeText, { color: C.text }]}>{t.soloComplete}</Text>
          <Text style={[styles.subText, { color: C.textMuted }]}>
            {config?.totalQuestions}q · {config?.difficulty} · {config?.operation}
          </Text>
        </Animated.View>
        ...
        >
          ...
          <View style={[styles.nameBadge, { backgroundColor: C.p1Primary }]}>
          ...
          <Text style={[styles.accuracyText, { color: accuracy >= 80 ? C.timerGreen : accuracy >= 60 ? C.timerYellow : C.timerRed }]}>
            {accuracy}% accuracy
          </Text>
        ...
        <Animated.View style={{ opacity: cardOpacity, gap: 12 }}>
          <TouchableOpacity style={styles.playAgainBtn} onPress={handlePlayAgain} activeOpacity={0.85}>
            <LinearGradient colors={['#4361EE', '#7C3AED']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  playAgainBtn: { borderRadius: 18, overflow: 'hidden', shadowColor: '#4361EE', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 6 },
