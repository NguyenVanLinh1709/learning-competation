...
function StatRow({
  label, value, highlight, textColor, mutedColor,
}: {
  ...
  return (
    <View style={statStyles.row}>
      ...
      <Text style={[statStyles.value, { color: highlight ? '#16A34A' : textColor }]}>{value}</Text>
    </View>
  );
}

function PlayerCard({
  player, isWinner, gradient, labels, textColor, mutedColor, borderColor,
}: {
  player: PlayerState;
  isWinner: boolean;
  gradient: readonly [string, string];
  labels: { points: string; correct: string; wrong: string; avgTime: string };
  textColor: string;
  mutedColor: string;
  borderColor: string;
}) {
  return (
    <LinearGradient
      ...
      style={[cardStyles.card, { borderColor }, isWinner && cardStyles.winnerCard]}
      start={{ x: 0, y: 0 }}
      end={{ x: 1, y: 1 }}
    >
      ...
      <Text style={[cardStyles.scoreLabel, { color: 'rgba(255,255,255,0.5)' }]}>{labels.points}</Text>
      <View style={cardStyles.divider} />
      <StatRow label={labels.correct} value={String(player.correctCount)} highlight={player.correctCount > 0} textColor="#FFFFFF" mutedColor="rgba(255,255,255,0.6)" />
      <StatRow label={labels.wrong}   value={String(player.wrongCount)}   textColor="#FFFFFF" mutedColor="rgba(255,255,255,0.6)" />
      <StatRow label={labels.avgTime} value={avgTime(player.responseTimes)} textColor="#FFFFFF" mutedColor="rgba(255,255,255,0.6)" />
    </LinearGradient>
  );
}
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.banner, { transform: [{ scale: bannerScale }] }]}>
          ...
          <Text style={[styles.subText, { color: C.textMuted }]}>
            {config?.totalQuestions} · {config?.difficulty} · {config?.operation}
          </Text>
        </Animated.View>

        {/* Cards */}
        <Animated.View style={[styles.cards, { opacity: cardsOpacity, transform: [{ translateY: cardsY }] }]}>
          <PlayerCard player={player2} isWinner={!isDraw && !p1Wins} gradient={G.p2Button} labels={statLabels} textColor={C.text} mutedColor={C.textMuted} borderColor={C.border} />
          <PlayerCard player={player1} isWinner={!isDraw && p1Wins}  gradient={G.p1Button} labels={statLabels} textColor={C.text} mutedColor={C.textMuted} borderColor={C.border} />
        </Animated.View>
        ...
        <Animated.View style={{ opacity: cardsOpacity }}>
          <TouchableOpacity style={styles.rematchBtn} onPress={handleRematch} activeOpacity={0.85}>
            <LinearGradient colors={['#4361EE', '#F72585']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  cards: { gap: 12 },
  rematchBtn: { borderRadius: 18, overflow: 'hidden', shadowColor: '#4361EE', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 6 },
...
const cardStyles = StyleSheet.create({
  ...
  winnerCard: { borderColor: 'rgba(255,255,255,0.3)', shadowColor: '#F72585', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 6 },
