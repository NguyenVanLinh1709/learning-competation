...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'ColorMemoryResult'> };
...
export default function ColorMemoryResultScreen({ navigation }: Props) {
  const { config, score, correctCount, wrongCount, responseTimes, resetGame, initGame } = useColorMemoryStore();
  ...
  const handlePlayAgain = () => {
    ...
    initGame();
    navigation.replace('ColorMemoryGame');
  };
      ...
      <View style={[styles.scroll, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 40 }]}>
        ...
        <Animated.View style={{ opacity: cardOpacity, gap: 12 }}>
          <TouchableOpacity style={styles.playAgainBtn} onPress={handlePlayAgain} activeOpacity={0.85}>
            <LinearGradient colors={['#F97316', '#FB923C']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
