...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'VocabCountdown'> };

const STEPS = ['3', '2', '1', 'GO!'];
const STEP_MS = 900;

export default function VocabCountdown({ navigation }: Props) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (current < STEPS.length) {
        ...
        initGame();
        setTimeout(() => navigation.replace('VocabGame'), 500);
      }
    };
    ...
    <LinearGradient colors={G.home} style={styles.container}>
      ...
      <Animated.Text
        style={[
          styles.countText,
          isGo && styles.goText,
          { color: isGo ? '#10B981' : C.text, transform: [{ scale }], opacity },
        ]}
      >
        {STEPS[stepIndex]}
      </Animated.Text>
