...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'ColorBattleResult'> };
...
export default function ColorBattleResultScreen({ navigation }: Props) {
  const { player1, player2, config, resetGame, initGame } = useColorBattleStore();
  ...
  const handleRematch = () => {
    ...
    initGame();
    navigation.replace('ColorBattleGame');
  };
    ...
    <LinearGradient colors={G.home} style={styles.outer}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.cards, { opacity: cardsOpacity, transform: [{ translateY: cardsY }] }]}>
          ...
          <PlayerCard player={player1} isWinner={!isDraw && p1Wins}  gradient={['#7C3AED', '#A855F7']} t={statLabels} />
        </Animated.View>
        ...
        <Animated.View style={{ opacity: cardsOpacity }}>
          <TouchableOpacity style={styles.rematchBtn} onPress={handleRematch} activeOpacity={0.85}>
            <LinearGradient colors={['#7C3AED', '#F72585']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  rematchBtn: {
    ...
    shadowColor: '#7C3AED', shadowOffset: { width: 0, height: 5 },
