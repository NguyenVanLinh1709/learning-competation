...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'FlagSoloResult'> };
...
export default function FlagSoloResultScreen({ navigation }: Props) {
  const { config, score, correctCount, wrongCount, responseTimes, resetGame, initGame } = useFlagSoloStore();
  ...
  useEffect(() => {
    Haptics.notificationAsync(
      accuracy >= 80 ? Haptics.NotificationFeedbackType.Success : Haptics.NotificationFeedbackType.Warning,
    );
  ...
  const handlePlayAgain = () => {
    ...
    initGame();
    navigation.replace('FlagSoloGame');
  };
    ...
    <LinearGradient colors={G.home} style={styles.outer}>
      <View style={[styles.inner, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 40 }]}>
        ...
        <Animated.View style={[styles.banner, { transform: [{ scale: bannerScale }] }]}>
          ...
          <Text style={[styles.completeText, { color: C.text }]}>{t.flagSoloComplete}</Text>
        ...
        {/* Stats card */}
        <Animated.View
          style={[styles.card, { backgroundColor: C.surface, borderColor: C.border, opacity: cardOpacity, transform: [{ translateY: cardY }] }]}
        >
        ...
        <Animated.View style={{ opacity: cardOpacity, gap: 12 }}>
          <TouchableOpacity style={styles.playAgainBtn} onPress={handlePlayAgain} activeOpacity={0.85}>
            <LinearGradient colors={[ACCENT, '#14B8A6']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 64, height: 64, borderRadius: 32, marginBottom: 10, borderWidth: 2, borderColor: 'rgba(255,255,255,0.25)' },
  ...
  playAgainBtn: {
    ...
    shadowColor: ACCENT, shadowOffset: { width: 0, height: 5 },
