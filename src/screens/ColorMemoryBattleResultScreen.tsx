...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'ColorMemoryBattleResult'> };
...
export default function ColorMemoryBattleResultScreen({ navigation }: Props) {
  ...
  const handleRematch = () => {
    ...
    initGame();
    navigation.replace('ColorMemoryBattleGame');
  };
    ...
    <LinearGradient colors={G.home} style={styles.outer}>
      <ScrollView contentContainerStyle={[styles.scroll, { paddingTop: insets.top + 20, paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.cards, { opacity: cardsOpacity, transform: [{ translateY: cardsY }] }]}>
          ...
          <PlayerCard player={player1} isWinner={!isDraw && p1Wins}  gradient={['#F97316', '#FB923C']} t={statLabels} />
        </Animated.View>
        ...
        <Animated.View style={{ opacity: cardsOpacity }}>
          <TouchableOpacity style={styles.rematchBtn} onPress={handleRematch} activeOpacity={0.85}>
            <LinearGradient colors={['#F97316', '#F72585']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
