...
type Props = { navigation: NativeStackNavigationProp<RootStackParamList, 'MemoryBattleResult'> };
...
export default function MemoryBattleResultScreen({ navigation }: Props) {
  const { player1, player2, config, resetGame, initGame } = useMemoryBattleStore();
  ...
  const handleRematch = () => {
    ...
    initGame();
    navigation.replace('MemoryBattleGame');
  };
    ...
    <LinearGradient colors={G.home} style={styles.outer}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.cards, { opacity: cardsOpacity, transform: [{ translateY: cardsY }] }]}>
          <PlayerCard player={player2} isWinner={!isDraw && !p1Wins} gradient={['#F72585', '#B5179E']} t={statLabels} />
          <PlayerCard player={player1} isWinner={!isDraw && p1Wins}  gradient={['#6366F1', '#818CF8']} t={statLabels} />
        </Animated.View>
        ...
        <Animated.View style={{ opacity: cardsOpacity }}>
          <TouchableOpacity style={styles.rematchBtn} onPress={handleRematch} activeOpacity={0.85}>
            <LinearGradient colors={['#6366F1', '#F72585']} style={styles.btnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
...
const styles = StyleSheet.create({
  ...
  rematchBtn: {
    ...
    shadowColor: '#6366F1', shadowOffset: { width: 0, height: 5 },
